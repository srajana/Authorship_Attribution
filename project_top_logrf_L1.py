...
def convert_to_svm(excerpts, labels, outfile, vocab):
	...
	for i,entry in enumerate(excerpts):
		...
		tokens = sum(freq_dict.values())
		x = np.array(freq_dict.values())
		den = np.linalg.norm(x,ord=1)
		for k in freq_dict.keys():
			freq_dict[k] = freq_dict[k]/tokens
			freq_dict[k] = math.log(freq_dict[k])
			freq_dict[k] = freq_dict[k]/den
		for w in new_words:
